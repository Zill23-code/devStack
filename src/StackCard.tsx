...
const StackCard = ({ techCard, setTechCard }: IStackCardProps) => {
  // Remove  
  const handleRemove = (removeBtn: string) => {
    setTechCard(techCard.filter((removeItem) => removeItem.id !== removeBtn));
  };
          ...
          <div className="space-y-2.5 max-h-[400px] overflow-y-auto pr-1 mb-4">
            {techCard.map((iCard) => (
              ...
              >
                ...
                <button
                  onClick={() => handleRemove(iCard.removeBtn)}
                  aria-label={`Remove ${iCard.name}`}
                  className="w-6 h-6 flex items-center justify-center rounded-full text-gray-400 hover:text-red-600 hover:bg-red-50 text-base font-bold transition-all leading-none"
                >
